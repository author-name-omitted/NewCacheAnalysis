...
- ****
    ...
    - parallels 、task，: 
- ****
  - _Pragma，clang，bug free
  - ，
  - papa bench
  - Mälardalen WCET benchmarks (MRTC)
- ****
  - sequential/adpcm_enc Assertion `0 && "We have unhandled pseudo instructions"' failed.
  - entry pointxxx_main，tacle-bench，loop
  - sequentialbench，，:ammunition, anagram, cjpeg_transupp, epic, huff_enc, susan 
  - sequential/gsm_enc.c 1867bound，8
  - https://github.com/RTS-SYSU/Timing-Analysis-Multicores
- ****
  - tacle-benchrecursion(UrGraph)
  - entrypoint(task，)(hopefully we can handle)
  - DEBIE，
...
# cd testcase(run.pytacle-bench)
if args.p:
    handle_generate(args) # ./run.py -p -s test
elif args.tacle: # tacle-bench
    # tacle-bench
    # - CoreInfo.json
    # - app/ kernel/ ...
