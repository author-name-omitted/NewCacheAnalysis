...
- **分类**
    - kernel小程序: 全部标注
    - sequential略大的程序: 大部分标注
    - app真实应用，如升降机、车窗控制: 全部标注
    - test自动生成的压测程序: 全部标注
    - parallels 多线程、多task，应该分析不了: 未标注
- **特点**
  - 存在_Pragma的循环标注，超哥之前通过修改clang来获取，但我暂未找到bug free的版本
  - 自包含，不依赖于其它库
  - 除了papa bench存在对特定架构的依赖
  - Mälardalen WCET benchmarks (MRTC)相对来说程序较小
- **备注**
  - sequential/adpcm_enc出现 Assertion `0 && "We have unhandled pseudo instructions"' failed.直接跳过了先
  - entry point全部取xxx_main，与其tacle-bench自带注释一致，故不可达的loop没有标注
  - sequential有的bench循环几十个要手动标，先不标了，先排除了:ammunition, anagram, cjpeg_transupp, epic, huff_enc, susan 
  - sequential/gsm_enc.c 1867行没给精确bound，直接取8了
  - 大部分标注由https://github.com/RTS-SYSU/Timing-Analysis-Multicores修改脚本后生成
- **比较特殊的情况**
  - tacle-bench中有递归的函数recursion(这种UrGraph应该会无穷扩展)
  - 存在多个entrypoint(如果不是指不同核不同task的话，目前的支持应该比较弱)和代码段共享(hopefully we can handle)
  - 这个DEBIE可以直接定义中断处理函数的，估计是嵌入式裸机开发
...
# csl add
parser.add_argument('-meta', action='store_true', help='use meta data loop bound, which means _Pragma from tacle-bench')
parser.add_argument('-tacle', action='store_true', help='generate bound for tacle-bench')
parser.add_argument('-taclebatch', action='store_true', help='generate bound for tacle-bench')
...
def handle_generate_tacle(args):
    src_dir = Path(os.path.abspath(args.src))
    if not src_dir.exists():
        logger_error(f'Source directory {src_dir} does not exist')
        exit(1)
    ...
    cf = Path(src_dir.parent.parent / args.core_info) # in tacle-bench/
    if not cf.exists():
        logger_error(f'Core info file {str(cf)} does not exist, please check the file name')
        exit(1)
...
def handle_generate_tacle_batch(args):
    batch_dir = Path(os.path.abspath(args.src))
    subdirs = [p for p in batch_dir.iterdir() if p.is_dir()]
    skip_bench = ["adpcm_enc"]
    for src_dir in subdirs:
        logger_info(f'======Enter {src_dir}======')
        if not src_dir.exists():
            logger_error(f'Source directory {src_dir} does not exist')
            exit(1)
        ...
        if src_dir.name in skip_bench:
            logger_error(f'skip {src_dir.name}')
            continue    
        # 需要处理entry point
        last_dir = src_dir.name  # 'prime'
        new_func_name = f"{last_dir}_main"  # 'prime_main'
        cf = Path(src_dir.parent.parent / args.core_info) # in tacle-bench/
        json_path = str(cf)
        ...
        for core_entry in data:
            for task in core_entry.get("tasks", []):
                task["function"] = new_func_name
...
def handle_meta(args):
    ...
    command = [
        ...
        "--ta-use-metadata-as-loop-annotation=true",
        "--ta-num-callsite-tokens=1",
        f"--core-info={str(cf)}",
        f"--core-numbers={args.num_cores}",
        "--shared-cache-Persistence-Analysis=false",
    ...
    logger_info(f'Running llvmta with the following arguments:')
    logger_info(f'Using loop from _Pragma')
...
# cd testcase(run.py和tacle-bench所在文件夹)
if args.p:
    handle_generate(args) # ./run.py -p -s test
elif args.tacle: # tacle-bench遵循下述文件结构直接可用
    # tacle-bench
    # - CoreInfo.json
    # - app/ kernel/ ...
    # ./run.py -tacle -s tacle-bench/kernel/binarysearch -n 1
    handle_generate_tacle(args) 
    # 输出在.c所在源文件，./out也有，可以不管
elif args.taclebatch:
    # ./run.py -taclebatch -s tacle-bench/kernel -n 1
    handle_generate_tacle_batch(args)
elif args.meta:
    handle_meta(args) # ./run.py -meta -s test
else:
    handle_run(args) # ./run.py -s test#!/usr/bin/env python3
