...
public:
  ...
  std::vector<std::vector<Ceop>>
  generate_all_combinations(const std::vector<std::string> &ifnlist) {
    std::vector<std::vector<Ceop>> all_combinations;
    std::vector<Ceop> current;

    // 
    std::function<void(int)> dfs = [&](int depth) {
      if (depth == ifnlist.size()) {
        all_combinations.push_back(current);
        return;
      }

      const std::string &cfn = ifnlist[depth];
      auto it = f2ceopsforCR.find(cfn);
      if (it != f2ceopsforCR.end()) {
        for (const Ceop &ceop : it->second) {
          current.push_back(ceop);
          dfs(depth + 1);
          current.pop_back();
        }
      }
  ...
  int run_M(std::string afn, std::vector<std::string> ifnlist) {
    int res = 0;
    unsigned tmp_cnt = 0; // for output
    for (Ceop &ceop : f2ceopsforCR[afn]) {
      CRs crlist(ceop);
      ...
      std::vector<std::vector<Ceop>> combinations =
          generate_all_combinations(ifnlist);
      for (std::vector<Ceop> &comb : combinations) {

        int t = crlist.DpMaxOutNumber(comb);
        std::ofstream myfile;
        myfile.open("ourM_OUTPUT.txt", std::ios_base::app);
        myfile << afn << ": " << t << std::endl;
        myfile.close();
  ...
  void analis(std::vector<std::vector<std::string>> &coreinfo) {
    ...
    if (coreinfo.size() == 2) {
      ...
      for (int i = 0; i < coreinfo.size(); i++) {
        ...
        for (int j = 0; j < coreinfo.size(); j++) {
          if (i == j)
            continue;
          temp.emplace_back(coreinfo[j][0]);
        }
        res = run_M(coreinfo[i][0], temp);
        std::cout << coreinfo[i][0] << " WCEET: " << res * Latency << std::endl;
        myfile << coreinfo[i][0] << " inter " << res * Latency << std::endl;
        myfile.close();
      }
    }
  }
  ...
  struct CRs {
    ...
    int DpMaxOutNumber(Ceop Urs) {
      ...
      int maxVal = 0;
      for (auto &states : Dp)
        for (int val : states.second)
          maxVal = std::max(maxVal, val);
      return maxVal;
    }

    void dfs(const std::vector<int> &lengths, int dim,
      ...
      for (int s = 0; s < len; ++s) {
        for (int e = s; e < len; ++e) {
          current[dim] = {s, e};
          dfs(lengths, dim + 1, current, result);
        }
      }
    }

    std::vector<std::vector<std::pair<int, int>>>
    enumerate_all_ranges(const std::vector<Ceop> &ceops) {
      int n = ceops.size();
      std::vector<int> lengths(n);
      for (int i = 0; i < n; ++i)
        lengths[i] = ceops[i].size();

      std::vector<std::vector<std::pair<int, int>>> result;
      std::vector<std::pair<int, int>> current(n);
      dfs(lengths, 0, current, result);
      return result;
    }

    int DpMaxOutNumber(std::vector<Ceop> ceops) {
      // 
      int maxVal = 0;
      std::vector<BlockInfo> SameBlock, lastSameBlock;
      std::map<int, std::map<std::vector<int>, int>> Dp, lastDp;
      Dp[0]; // 
      for (int i = 0; i < crs.size(); ++i) {
        CR cr = crs[i];
        swap(lastSameBlock, SameBlock); // DP
        ...
        for (auto &Sate : lastDp) {
          ...
          for (int id = 0; id < SameBlock.size(); ++id)
            if (s.count(SameBlock[id].address))
              j1 |= (1 << id); // jblock addr
          std::vector<std::vector<std::pair<int, int>>> Rangs =
              enumerate_all_ranges(ceops);

          int j2 = j1;
          for (std::vector<std::pair<int, int>> &rangs : Rangs) {
            // UR urcopy = UR();
            std::vector<std::vector<UR>> urs2c;
            std::vector<int> location;
            std::vector<int> lastlocation;
            for (int num = 0; num < ceops.size(); num++) {
              std::vector<UR> temp;
              temp.insert(temp.end(), ceops[num].begin() + rangs[num].first,
                          ceops[num].begin() + rangs[num].second + 1);
              urs2c.emplace_back(temp);
              location.emplace_back(rangs[num].second);
              lastlocation.emplace_back(rangs[num].first);
            }

            int totalOut1 =
                cr.OutNumber_NotPersistBlock(j2, SameBlock, s, urs2c);
            std::vector<int> PersistOut =
                cr.OutNumber_PersistentBlock_add_with_continue(urs2c);
            int totalOut = 0;
            for (auto val : PersistOut) {
              assert(val >= 0 && "0");
              totalOut += val;
            ...
            Dp[j2][location] =
                std::max(Dp[j2][location],
                         lastDp[j][lastlocation] + totalOut1 + totalOut);
            maxVal = std::max(maxVal, Dp[j2][location]);
          }
        }
      }
      return maxVal;
